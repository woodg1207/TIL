...
// 2-2 .map 을 사용해 images 배열 안의 Object 들의 height들만 저장되어 있는 heights배열을 만드시오.
const images = [{
    height: '34px',
    width: '39px'
  },
...
// })
images.map(image => heights.push(image.height))
console.log(heights)

//2-3 map 을 사용해 trips 배열의 값들을 계산해서 속도 값을 저장하는 배열 speeds를 만드시오.
const trips = [{
    distance: 35,
    time: 10
  },
...
speeds = []
trips.map(trip => speeds.push(trip.distance / trip.time))
console.log(speeds)
